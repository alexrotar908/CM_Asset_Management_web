...
export default function HotelApartmentDubai() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Todas las propiedades cuyo tipo tenga slug 'hotel-apartment' en la ciudad de Dubai City.
        ...
        const mapped: Property[] = (data ?? []).map((p: any) => {
          ...
          // Ordenamos imágenes por id para consistencia
        ...
        // Orden estable por título en cliente
        ...
        if (!mounted) return;
        setError(e?.message || 'Error al cargar áticos');
      } finally {
        if (!mounted) return;
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="hotel_dubai-page">
        <div className="hotel_dubai-content"><h2>Cargando hoteles en Dubai City…</h2></div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="hotel_dubai-page">
        <div className="hotel_dubai-content"><h2>Error: {error}</h2></div>
      </div>
    );
  }
  ...
  return (
    <div className="hotel_dubai-page">
      <div className="hotel_dubai-content">
        {/* Breadcrumb */}
        <div className="breadcrumb">
          <Link to="/" className="breadcrumb-link">Home</Link>
          <span className="breadcrumb-separator">{'>'}</span>
          <span className="breadcrumb-current">Hoteles</span>
        </div>

        <div className="hotel_dubai-header">
          <h1>Hoteles</h1>
          <div className="hotel_dubai-subheader">
        ...
        <div className="hotel_dubai-grid">
          {paginated.map((prop) => (
            <div
              key={prop.id}
              className="hotel_dubai-card clickable"
      ...
      <footer className="hotel_dubai-footer">
