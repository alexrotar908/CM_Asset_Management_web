// src/pages/tipo/malaga_type/attic_penthouse/attic_penthouse.tsx
import { useEffect, useMemo, useRef, useState } from 'react';
import './attic_penthouse.css';
...
export default function AtticPenthouse() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        setError(e?.message || 'Error al cargar Attic Penthouse (Málaga)');
      } finally {
        if (!mounted) return;
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="attic-page">
        <div className="attic-content"><h2>Cargando propiedades Attic Penthouse (Málaga)…</h2></div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="attic-page">
        <div className="attic-content"><h2>Error: {error}</h2></div>
      </div>
    );
  }
  ...
  return (
    <div className="attic-page">
      <div className="attic-content">
        {/* Breadcrumb */}
        <div className="breadcrumb">
          <Link to="/" className="breadcrumb-link">Home</Link>
          <span className="breadcrumb-separator">{'>'}</span>
          <span className="breadcrumb-current">Attic Penthouse</span>
        </div>

        <div className="attic-header">
          <h1>Attic Penthouse</h1>
          <div className="attic-subheader">
        ...
        <div className="attic-grid">
          {paginated.map((prop) => (
            <div key={prop.id} className="attic-card">
              <div
