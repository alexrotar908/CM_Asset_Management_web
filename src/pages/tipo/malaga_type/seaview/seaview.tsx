// src/pages/tipo/malaga_type/seaview/seaview.tsx
...
export default function Seaview() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Trae TODAS las propiedades de Málaga con tipo 'seaview' (sin filtrar por categoría)
        ...
        const mapped: Property[] = (data ?? []).map((p: any) => {
          ...
          // Portada primero + resto, sin duplicados
        ...
        if (!mounted) return;
        setError(e?.message || 'Error al cargar Seaview (Málaga)');
      } finally {
        if (!mounted) return;
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="seaview-page">
        <div className="seaview-content"><h2>Cargando propiedades Seaview (Málaga)…</h2></div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="seaview-page">
        <div className="seaview-content"><h2>Error: {error}</h2></div>
      </div>
    );
  }
  ...
  return (
    <div className="seaview-page">
      <div className="seaview-content">
        {/* Breadcrumb */}
        <div className="breadcrumb">
          <Link to="/" className="breadcrumb-link">Home</Link>
          <span className="breadcrumb-separator">{'>'}</span>
          <span className="breadcrumb-current">Sea View</span>
        </div>

        <div className="seaview-header">
          <h1>Sea View </h1>
          <div className="seaview-subheader">
        ...
        <div className="seaview-grid">
          {paginated.map((prop) => (
            <div key={prop.id} className="seaview-card">
              <div
      ...
      <footer className="seaview-footer">
