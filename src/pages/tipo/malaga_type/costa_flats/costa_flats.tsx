// src/pages/tipo/malaga_type/costa_flats/costa_flats.tsx
import { useEffect, useMemo, useRef, useState } from 'react';
import './costa_flats.css';
...
export default function CostaFlats() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        setError(e?.message || 'Error al cargar Costa Flats (Málaga)');
      } finally {
        if (!mounted) return;
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="costa-page">
        <div className="costa-content"><h2>Cargando propiedades Costa Flats (Málaga)…</h2></div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="costa-page">
        <div className="costa-content"><h2>Error: {error}</h2></div>
      </div>
    );
  }
  ...
  return (
    <div className="costa-page">
      <div className="costa-content">
        {/* Breadcrumb */}
        <div className="breadcrumb">
          <Link to="/" className="breadcrumb-link">Home</Link>
          <span className="breadcrumb-separator">{'>'}</span>
          <span className="breadcrumb-current">Costa Flats</span>
        </div>

        <div className="costa-header">
          <h1>Costa Flats (Málaga)</h1>
          <div className="costa-subheader">
        ...
        <div className="costa-grid">
          {paginated.map((prop) => (
            <div key={prop.id} className="costa-card">
              <div
