// src/pages/tipo/malaga_type/front_line_golf/front_line_golf.tsx
import { useEffect, useMemo, useRef, useState } from 'react';
import './front_line_golf.css';
...
export default function FrontLineGolf() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        setError(e?.message || 'Error al cargar Front Line Golf (Málaga)');
      } finally {
        if (!mounted) return;
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="golf-page">
        <div className="golf-content"><h2>Cargando Front Line Golf en Málaga…</h2></div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="golf-page">
        <div className="golf-content"><h2>Error: {error}</h2></div>
      </div>
    );
  }
    ...
    <div className="golf-page">
      <div className="golf-content">
        <div className="breadcrumb">
          ...
          <span className="breadcrumb-current">Front Line Golf</span>
        </div>

        <div className="golf-header">
          <h1>Front Line Golf</h1>
          <div className="golf-subheader">
        ...
        <div className="golf-grid">
          {paginated.map((prop) => (
            <div key={prop.id} className="golf-card">
              <div
