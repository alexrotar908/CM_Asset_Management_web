// src/pages/tipo/malaga_type/front_line_beach/front_line_beach.tsx
import { useEffect, useMemo, useRef, useState } from 'react';
import './front_line_beach.css';
...
interface Property {
  ...
  size: number;
  garage?: number;
  province: string; // aquí guardaremos la ciudad (Málaga)
  area: string;
  typeSlug?: string;
  featured?: boolean;
  status: Status;
...
export default function FrontLineBeach() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Todas las propiedades del tipo `front_line_beach` en la ciudad de Málaga
        const { data, error } = await supabase
          ...
          `)
          .eq('tipos.slug', 'front_line_beach')
          .in('zonas.ciudad', ['Málaga', 'Malaga']); // tolerante a tildes
        ...
        const mapped: Property[] = (data ?? []).map((p: any) => {
          ...
          // Portada primero + resto de galería, sin duplicados
        ...
        if (!mounted) return;
        setError(e?.message || 'Error al cargar Front Line Beach (Málaga)');
      } finally {
        if (!mounted) return;
        setLoading(false);
      }
  ...
  const onWheel = (id: string, e: React.WheelEvent<HTMLDivElement>) => {
    if (e.deltaY > 0) nextImage(id); else prevImage(id);
  };
  ...
  if (loading) {
    return (
      <div className="frontline-page">
        <div className="frontline-content"><h2>Cargando propiedades Front Line Beach (Málaga)…</h2></div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="frontline-page">
        <div className="frontline-content"><h2>Error: {error}</h2></div>
      </div>
    );
  }
    ...
    <div className="frontline-page">
      <div className="frontline-content">
        ...
        <div className="breadcrumb">
          ...
          <span className="breadcrumb-current">Front Line Beach</span>
        </div>

        <div className="frontline-header">
          <h1>Front Line Beach</h1>
          <div className="frontline-subheader">
        ...
        <div className="frontline-grid">
          {paginated.map((prop) => (
            <div key={prop.id} className="frontline-card">
              <div
              ...
              >
                ...
                <span className="status-label buy">BUY</span>
              </div>
      ...
      <footer className="frontline-footer">
