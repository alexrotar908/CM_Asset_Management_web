// src/pages/tipo/madrid_type/commercial/commercial.tsx
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Todos los "commercial" en Madrid (sin filtrar por categoría).
        ...
        const mapped: Property[] = (data ?? []).map((p: any) => {
          ...
          // Ordena imágenes por id para consistencia
        ...
        if (!mounted) return;
        setItems(stable);
        setImageIndexes(Object.fromEntries(stable.map((p) => [p.id, 0])));
      } catch (e: any) {
        if (!mounted) return;
        setError(e?.message || 'Error al cargar propiedades commercial');
      } finally {
        if (!mounted) return;
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="commercial-page">
        <div className="commercial-content"><h2>Cargando Commercial (Madrid)…</h2></div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="commercial-page">
        <div className="commercial-content"><h2>Error: {error}</h2></div>
      </div>
    );
  }
    ...
    <div className="commercial-page">
      <div className="commercial-content">
        ...
        <div className="breadcrumb">
          ...
          <span className="breadcrumb-current">Commercial</span>
        </div>

        <div className="commercial-header">
          <h1>Commercial</h1>
          <div className="commercial-subheader">
        ...
        <div className="commercial-grid">
          {paginated.map((prop) => (
            <div
              key={prop.id}
              className="commercial-card clickable"
