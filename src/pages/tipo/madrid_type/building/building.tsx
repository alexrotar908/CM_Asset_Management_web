// src/pages/tipo/madrid_type/building/building.tsx
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Todos los edificios en Madrid (tipos.slug = 'building'), sin filtrar por categoría
        ...
        // Orden estable en cliente (evitamos order() sobre relaciones deprecado)
        ...
        if (!mounted) return;
        setError(e?.message || 'Error al cargar edificios');
      } finally {
        if (!mounted) return;
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <div className="building-page">
        <div className="building-content"><h2>Cargando edificios en Madrid…</h2></div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="building-page">
        <div className="building-content"><h2>Error: {error}</h2></div>
      </div>
    );
  }
    ...
    <div className="building-page">
      <div className="building-content">
        ...
        <div className="breadcrumb">
          ...
          <span className="breadcrumb-current">Building (Madrid)</span>
        </div>

        <div className="building-header">
          <h1>Building (Madrid)</h1>
          <div className="building-subheader">
        ...
        <div className="building-grid">
          {paginated.map((prop) => (
            <div
              key={prop.id}
              className="building-card clickable"
