// src/zonas/centro_historico.tsx
import { useEffect, useMemo, useRef, useState } from 'react';
import './centro_historico.css';
...
export default function CentroHistorico() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // TODAS las propiedades del Centro Histórico (sin filtrar por categoría/tipo)
        const { data, error } = await supabase
          ...
          `)
          .in('zonas.ciudad', ['Málaga', 'Malaga'])
          .in('zonas.area', ['Centro Histórico', 'Centro Historico'])
        ...
        const mapped: Property[] = (data ?? []).map((p: any) => {
          ...
          // Ordena imágenes por id en cliente para consistencia
          ...
          const ordered = [
            p.imagen_principal || '',
            ...gallery.filter((u) => u && u !== p.imagen_principal),
          ].filter(Boolean);
          ...
          return {
            ...
            area: zona?.area ?? 'Centro Histórico',
        ...
        setItems(mapped);
        setImageIndexes(Object.fromEntries(mapped.map((p) => [p.id, 0])));
      } catch (e: any) {
        if (!mounted) return;
  ...
  if (loading) {
    return (
      <div className="centro_historico-page">
        <div className="centro_historico-content"><h2>Cargando propiedades del Centro Histórico…</h2></div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="centro_historico-page">
        <div className="centro_historico-content"><h2>Error: {error}</h2></div>
      </div>
    );
  }
  ...
  return (
    <div className="centro_historico-page">
      <div className="centro_historico-content">
        {/* Breadcrumb */}
        <div className="breadcrumb">
          <Link to="/" className="breadcrumb-link">Home</Link>
          <span className="breadcrumb-separator">{'>'}</span>
          <span className="breadcrumb-current">Centro Histórico</span>
        </div>

        <div className="centro_historico-header">
          <h1>Centro Histórico</h1>
          <div className="centro_historico-subheader">
        ...
        <div className="centro_historico-grid">
          {paginated.map((prop) => (
            <div
              key={prop.id}
              className="centro_historico-card clickable"
      ...
      <footer className="centro_historico-footer">
