...
export default function Chamberi() {
    ...
    (async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .in('zonas.area', ['Chamberí', 'Chamberi'])
        ...
        const mapped: Property[] = (data ?? []).map((p: any) => {
          ...
          const gallery = imgs.map((i) => i?.url).filter(Boolean) as string[];
          const ordered = [
            p.imagen_principal || '',
  ...
  const sorted = useMemo(() => {
    switch (sortOption) {
      case 'lowToHigh': return [...items].sort((a, b) => a.price - b.price);
      case 'highToLow': return [...items].sort((a, b) => b.price - a.price);
  ...
  if (loading) {
    return <div className="chamberi-page"><div className="chamberi-content"><h2>Cargando propiedades de Chamberí…</h2></div></div>;
  }
  if (error) {
    return <div className="chamberi-page"><div className="chamberi-content"><h2>Error: {error}</h2></div></div>;
  }

  return (
    <div className="chamberi-page">
      <div className="chamberi-content">
        <div className="breadcrumb">
          <Link to="/" className="breadcrumb-link">Home</Link>
          <span className="breadcrumb-separator">{'>'}</span>
          <span className="breadcrumb-current">Chamberí</span>
        </div>

        <div className="chamberi-header">
          <h1>Chamberí</h1>
          <div className="chamberi-subheader">
        ...
        <div className="chamberi-grid">
          {paginated.map((prop) => (
            <div
              key={prop.id}
              className="chamberi-card clickable"
            ...
            >
              ...
              >
                {prop.images.length > 0 && <img src={prop.images[imageIndexes[prop.id] || 0]} alt={prop.title} />}
      ...
      <footer className="chamberi-footer">
